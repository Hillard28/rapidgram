"""
Example matching function that uses rapidgram to perform fuzzy matching
"""
...
filepath = os.environ['USERPROFILE'] + "/rapidgram/"
sys.path.append(filepath)

from rapidgram import gram, pregram
            ...
            if (
                ...
                if type(mtype) == list:
                    ...
                    for lvar, rvar, thres, mt in zip(
                        ...
                        if mt == "full":
                            potential["fuzz_" + str(m)] = np.vectorize(gram.ratio) (potential[lvar], potential[rvar])
                        elif mt == "partial":
                            potential["fuzz_" + str(m)] = np.vectorize(gram.partial_ratio) (potential[lvar], potential[rvar])
                        elif mt == "token":
                            potential["fuzz_" + str(m)] = np.vectorize(gram.token) (potential[lvar], potential[rvar])
                ...
                if mtype == "full":
                    potential["total_score"] = np.vectorize(gram.ratio) (potential[left_fuzzy], potential[right_fuzzy])
                elif mtype == "partial":
                    potential["total_score"] = np.vectorize(gram.partial_ratio) (potential[left_fuzzy], potential[right_fuzzy])
                elif mtype == "token":
                    potential["total_score"] = np.vectorize(gram.token) (potential[left_fuzzy], potential[right_fuzzy])
